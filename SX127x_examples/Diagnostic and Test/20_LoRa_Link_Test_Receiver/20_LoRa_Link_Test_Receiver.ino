...
/*******************************************************************************************************
  ...
  The program is a matching receiver program for the '10_LoRa_Link_Test_TX'. The packets received are displayed
  on the serial monitor and analysed to extract the packet data which indicates the power used to send the
  packet. A count is kept of the numbers of each power setting received. When the transmitter sends the test
  mode packet at the beginning of the sequence (displayed as 999) the running totals of the powers received
  is printed. Thus you can quickly see at what transmit power levels the reception fails. 
...
uint32_t Test1Count[32];                         //buffer where counts of received packets are stored, -9dbm to +22dBm
...
void processPacket()
{
  ...
  if (packettype == TestPacket)
  {
    ...
    if (RXBUFFER[0] == '-')
    {
      lTXpower = ~lTXpower + 1;                    //invert all the bits make the power -ve
    }
    ...
    if (updateCounts)
    {
      temp = (Test1Count[lTXpower + 9]) + 1;
      Test1Count[lTXpower + 9] = temp;
    }
  }
