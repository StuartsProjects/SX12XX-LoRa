...
/*******************************************************************************************************
  ...
  will be transmitted until a valid acknowledge comes from the receiver. Use with the receiver programs,
  233_SDfile_Transfer_Transmitter.ino or 235_SDfile_Transfer_TransmitterIRQ.ino.
  ...
  This is a version of example 234_SDfile_Transfer_Receiver.ino that does not require the use of the DIO0 pin
  to check for transmit or receive done. In addition no NRESET pin is needed either, so its a program for use
  with a minimum pin count Arduino. Leave the DIO0 and NRESET pins on the LoRa device not connected.
...
bool receiveaPacketDT()
{
  ...
  if (RXPacketL > 0)
  {
    //if the LT.receiveDTIRQ() returns a value > 0 for RXPacketL then packet was received OK
...
void setup()
{
  ...
  Serial.begin(115200);
  Serial.println();
  Serial.println(F("236_SDfile_Transfer_ReceiverIRQ starting"));
