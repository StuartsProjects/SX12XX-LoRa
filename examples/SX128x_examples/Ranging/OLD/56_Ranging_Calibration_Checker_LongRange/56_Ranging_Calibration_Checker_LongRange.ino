...
void loop()
{
  ...
  for (index = CalibrationStart; index <= CalibrationEnd; index = index + 10)
  {
    ...
    Serial.print("TransmitRanging,Calibration,");
    Serial.print(index);
    ...
    if (IrqStatus & IRQ_RANGING_MASTER_RESULT_TIMEOUT)
    {
      ...
      Serial.print(",RangingTimeout");
    }

    if (millis() > endwaitmS)
    {
      Serial.print(",ProgramTimeout");
    }
...
void setup()
{
  ...
  //The 'Setup LoRa device for Ranging' list below can be replaced with a single function call;
