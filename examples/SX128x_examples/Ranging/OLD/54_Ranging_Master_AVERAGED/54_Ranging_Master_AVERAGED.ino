...
#ifdef ENABLEI2CBACKPACK
#include <Wire.h>
#include <I2CDisplay.h>
#define DisplayAddress 0x18
I2CDisplay disp(DisplayAddress);                            //create the display class
#endif
...
void loop()
{
  ...
  for (index = 1; index <= rangeingcount; index++)
  {
    ...
    if (IrqStatus & IRQ_RANGING_MASTER_RESULT_VALID)
    {
      ...
      range_result = LT.getRangingResultRegValue(RANGING_RESULT_AVERAGED);
      range_result_sum = range_result_sum + range_result;
      Serial.print(F(",RAW,"));
      Serial.print(range_result,HEX);
      distance = LT.getRangingDistance(RANGING_RESULT_AVERAGED, range_result, distance_adjustment);
...
void setup()
{
  ...
  Serial.println();
  Serial.println(F("54_Ranging_Master_AVERAGED Starting"));
