...
/*******************************************************************************************************
  ...
  and record non-ASCII packets. The LED will flash for each packet received. The first few bytes only of a
  packet and last 4 bytes are printed out.
...
void packet_is_OK()
{
  ...
  if (RXPacketL > 32)
  {
    LoRa.printSXBufferHEX(0, 15);
    Serial.print(F(" .. "));
    LoRa.printSXBufferHEX(RXPacketL - 4, (RXPacketL - 1));
  }
  else
  {
...
void setup()
{
  ...
  Serial.begin(115200);
  Serial.println(F("222_LoRa_DTPacket_Monitor Starting"));
