...
/*******************************************************************************************************
  ...
  ********************
  ...
  symbol time for the settings you are using. This example uses spreading factor 7 with a bandwidth of 
  125Khz. The symbol time is therefore 1024uS. 
  ...
  To implement a 100:1 reduction in the LoRa device receive current, the sleep time needs to be 100 times the  
  wake time so needs to be set to 930816uS.

  The transmitter, see program 2_LoRa_Transmitter_RXdutycycle_SF7BW125, must send a packet that has a preamble
  that is longer than the LoRa devices wake and sleep periods combined. The combined preamble time then needs
  to be 9216 + 930816uS = 940032uS. At 1024us per symbol that means the preamble needs to be 909 symbols.
  A couple of extra symbols will comepensate for some of the short delays as the LoRa device switches modes. 
    
  When the LoRa receiver detects a packet the LoRa device DIO1 signal wakes up the ESP32S3 from light sleep
  and then code initialises the SPI bus and LoRa device in such a way that the packet received is not lost.
  The packet is read and printed out. The board then goes back into light sleep with the LoRa receiver again
  in RX duty cycle mode. 
